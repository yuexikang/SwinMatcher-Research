...
WANDB_NAME="${WANDB_NAME:-SwinMatcherBIT_stage2_geometry_strong}"
...
cmd=(
  "${PYTHON_CMD[@]}" train.py
  --data_cfg_path configs/swinmatcherbit_stage2_512.py
  --main_cfg_path configs/swinmatcher_ds.py
  --exp_name SwinMatcherBIT_stage2_geometry_strong
  --gpus "$GPUS"
...
if [[ -n "$RESUME_CKPT_PATH" ]]; then
  ...
  if [[ -z "$CKPT_PATH" ]]; then
    echo "Usage: scripts/train_stage2.sh /path/to/stage1.ckpt"
    echo "Or resume with: RESUME_CKPT_PATH=/path/to/last.ckpt scripts/train_stage2.sh"
    exit 1
  fi
